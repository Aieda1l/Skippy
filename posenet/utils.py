...
def read_numpy(img, scale_factor=1.0, output_stride=16):
    return _process_input(img, scale_factor, output_stride)
...
def draw_skeleton(
        img, instance_scores, keypoint_scores, keypoint_coords,
        min_pose_confidence=0.5, min_part_confidence=0.5, color=(255, 255, 0)):
    out_img = img
    adjacent_keypoints = []
    for ii, score in enumerate(instance_scores):
        if score < min_pose_confidence:
    ...
    out_img = cv2.polylines(out_img, adjacent_keypoints, isClosed=False, color=color)
    return out_img


def draw_skel_and_kp(
        img, instance_scores, keypoint_scores, keypoint_coords,
        min_pose_score=0.5, min_part_score=0.5, color=(255, 255, 0)):
...
def get_keypoints(
    ...
    for ii, score in enumerate(instance_scores):
        ...
        for ks, kc in zip(keypoint_scores[ii, :], keypoint_coords[ii, :, :]):
            if ks < min_part_score:
                cv_keypoints[ii].append([0, 0])
                continue
            
            cv_keypoints[ii].append([kc[1], kc[0]])
